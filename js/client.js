...
function create()
{
  ...
  debugTextLeft = game.add.text(5, 5, "false");
  debugTextRight = game.add.text(width / 2 + 5, 5, "false");
...
function draw()
{
  controlLeft.draw();
  controlRight.draw();
  debugTextLeft.setText(controlLeft.isIn(game.input.x, game.input.y));
  debugTextRight.setText(controlRight.isIn(game.input.x, game.input.y));
}

function update()
{
  if (game.input.x < width / 2)
    controlLeft.set(game.input.x, game.input.y);
  else
    controlRight.set(game.input.x, game.input.y);
  draw();
}
